...
int main(void)
{
  ...
  if(SlaveHandle == NULL)
      HAL_UART_Transmit(&huart2, (uint8_t*)error1, sizeof(error1), 10);
  /* creation of Host */
  HostHandle = osThreadNew(HostTask, NULL, &Host_attributes);
    if(HostHandle == NULL)
        HAL_UART_Transmit(&huart2, (uint8_t*)error2, sizeof(error2), 10);
...
void HostTask(void *argument)
{
  ...
  for(;;)
  {
	  ...
	  for(int i = 0; i < REQUEST_SIZE(pc); i++) {
		  HAL_UART_Transmit(&huart4, data+  i, 1, 100);
		  taskYIELD();
	  }
